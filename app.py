...
@st.cache_resource
def train_model(df):
    fcols = ["גיל","BMI","גלוקוז","לחץ_דם","אינסולין","עובי_עור","הריונות","DPF","מעשן"]
    amap  = {"נמוכה":0,"בינונית":1,"גבוהה":2}
    X     = df[fcols].copy()
    X["פעילות_גופנית"] = df["פעילות_גופנית"].map(amap)
    y     = df["סוכרת"]
...
with tab2:
    ...
    with right:
        ...
        prob = model.predict_proba(scaler.transform(X_user))[0][1]
        pct  = prob * 100
